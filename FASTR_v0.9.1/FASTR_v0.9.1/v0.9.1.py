...
def main():    
    ...
    parser.add_argument('--sc', type=int, default=1000000,
                        help='sample reads cutoff: minimum number of reads required for a sample index to be valid; default = 1000000')
    parser.add_argument('--ro', type=int, default=500,
                        help='read offset: maximum deviation from the median alignment position of a group of reads with identical UMIs for a read to be valid')
    parser.add_argument('--po', type=int, default=50000000,
                        help='paired read offset: maximum difference in alignment position between two mates for a paired set of reads to be valid')
    ...
    with open('summary.txt','a') as f:
        f.write(summStr4) 
    t2 = time()
    print(f'run comepleted, {t2 - t0} s total')
